...
void Account::readAllUserName(vector<string>& userList)
{
    string s;
    ifstream fin;
    fin.open("User/User.txt");//打开用户名文件
    while (getline(fin, s))//逐行读取信息
        userList.emplace_back(s);
}
void Account::saveUserList(string username)
{
    ofstream fout;
    fout.open("User/User.txt", ios::app);//将数据写入文件
    fout << username << endl;
    fout.close();//关闭文件
}
...
{
    setBalance(getBalance() - good->getPiece());//发送快递需要花费
...
/*
* 将string类型对象转换为QString
* @param const string&型 待转换数据
* @return QString转换后的数据
*/
QString Account::sToq(const string& s)
{
    return QString(QString::fromLocal8Bit(s.c_str()));
}
/*
* 将QString类型对象转换为string
* @param const QString&型 待转换数据
* @return string转换后的数据
*/
string Account::qTos(const QString& s)
{
...
* 校验用户名合法性
*/
bool Account::checkUsername(string username, int index)
{
    bool result = true;
...
* 校验电话号码合法性
*/
bool Account::checkNumber(string number)
{
    bool result = false;
    if (number.length() == 11)
        result = true;
