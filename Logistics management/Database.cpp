...
{
	string sql = "select * from good";//sql语句，从goods表查询所有数据
...
vector<Goods> Database::findPackageByName(string name,int index)
{
	...
	if (index == 1)//参数为1，传入的是寄件人
		sql = "select * from good where sender = '" + name + "'";
	else if (index == 2)//参数为2，传入的是收件人
		sql = "select * from good where recipient = '" + name + "'";
	else //管理员查询
		sql = "select * from good where company ='" + name + "'";
	...
	while (column = mysql_fetch_row(res))//在已知字段数量情况下，获取下一行 
	{
		...
		g.setSendTime(column[2]);
		g.setReceiveTime(column[3]);
		g.setSender(column[4]);
		g.setRecipient(column[5]);
		g.setDecription(column[6]);
		g.setCompany(column[7]);
		//cout << column[0] << column[1] << column[2] << column[3];//返回一个所有字段结构的数组
		goods.push_back(g);//将读取的数据压入goods列表
...
{
	string sql = "insert into good values('"+ goods.getState(1)+"','"+ goods.getExpressNumber()+ "','" + goods.getSendTime(1)
		+ "','" +goods.getReceiveTime(1) + "','" +goods.getSender()+ "','" +goods.getRecipient()+ "','" + goods.getDescribtion()
		+"','" +goods.getCompany()+ "')";//sql插入语句，向goods表插入新数据
	mysql_query(&mysql, "set names gbk");
	mysql_query(&mysql, sql.data()); // 执行指定为一个空结尾的字符串的SQL查询。	
}
/*
* 快递信息修改函数
* 向数据库中修改已知快递信息
...
{
	string sql = "update good set state = '" + goods.getState(1)  +
		+"',sendTime='" + goods.getSendTime(1) + "',ReceiveTime='" + goods.getReceiveTime(1) + "',recipient='" 
		+ goods.getRecipient()+ "',description='" + goods.getDescribtion() +"',company='"+goods.getCompany()
		+ "'where expressnumber=" + goods.getExpressNumber();//sql更新语句，根据快递编码更新goods表中的快递信息
	mysql_query(&mysql, "set names gbk");
	mysql_query(&mysql, sql.data());// 执行指定为一个空结尾的字符串的SQL查询。	
}
/*
...
{
	string sql = "delete from good where expressnumber=" + goods.getExpressNumber();//sql删除语句，根据快递编码删除goods表中的对应信息
	mysql_query(&mysql, "set names gbk");
	mysql_query(&mysql, sql.data()); // 执行指定为一个空结尾的字符串的SQL查询。	
}
/*
...
* 根据公司名向管理员转账
*/
void Database::addBalance(string name)
{
...
//	cout << "结果行数为：" << mysql_affected_rows(&mysql) << endl;
	...
	mysql_query(&mysql, "set names gbk");
	mysql_query(&mysql, sql2.data()); // 执行指定为一个空结尾的字符串的SQL查询。	
